...
export const Searcher = forwardRef<HTMLDivElement, SearcherProps>(
  ({ inputBaseProps, darkVersion = false, searchText, onSearchTextChange, onSearch = undefined }, ref) => {
    return (
      <Box
        ref={ref}
        sx={{
          ...
          ":focus-within": {
            borderColor: theme =>
              darkVersion ? alpha(theme.palette.grey[100], 0.1) : theme.palette.common.darkGrayBackground,
      ...
      >
        ...
        />
        <IconButton sx={{ p: "5px", color: "inherit" }} aria-label="search" onClick={onSearch}>
...
const StyledInputBase = styled(InputBase)<{ darkVersion?: boolean }>(({ theme, darkVersion = false }) => ({
  "& .MuiInputBase-input": {
    padding: 0,
    width: "100%",
    fontSize: "14px",
