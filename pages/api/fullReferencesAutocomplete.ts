...
async function handler(req: NextApiRequest, res: NextApiResponse<ResponseAutocompleteFullTags>) {
    ...
    try {
        ...
        const searchResults = await clientTypesense.collections<LinkedKnowledgeNode>("fullReferences").documents().search(searchParameters);
        const tags: LinkedKnowledgeNode[] = (searchResults.hits || []).map(el => el.document);
        res.status(200).json({ results: tags || [] });
    } catch (error) {
        console.error(error);
        res.status(500).json({ errorMessage: "Cannot get references" });
    }
}
