...
describe("signalAllUserNodesChanges", () => {
  ...
  it("should signal the node changes to all the online users (or last_online < 24 hours) associated with the node.", async () => {
    let writeCounts = 0;
  ...
  it("should signal the major changes to all the users associated with the node.", async () => {
    let writeCounts = 0;
    const batch = db.batch();
    let { userNodesRefs, userNodesData }: any = await getAllUserNodes({ nodeId });
  ...
  it("should signal the delete changes to all the users associated with the node.", async () => {
    let writeCounts = 0;
    const batch = db.batch();
    let { userNodesRefs, userNodesData }: any = await getAllUserNodes({ nodeId });
